...
function Header() {
  ...
  const [newNotification, setNewNotification] = useState(false); // Trạng thái cho biểu tượng thông báo
  ...
  return (
    <header className="bg-blue-700 text-white py-4 md:py-6 px-4 md:px-8 flex justify-between items-center">
      ...
      <nav className="hidden md:flex space-x-6">
        ...
        <Link to="/news" className="text-lg font-semibold hover:underline">
          NEWS
        </Link>
        <Link to="/courses" className="text-lg font-semibold hover:underline">
          COURSE
        </Link>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button
          ...
          className="flex items-center space-x-2 bg-blue-500 hover:bg-blue-600 py-2 px-2 rounded-lg text-sm font-semibold transition duration-300 ring-2 ring-blue-400"
        >
          <ShoppingCartIcon className="h-5 w-5" />
          {cartCount > 0 && ( // Hiển thị số lượng sản phẩm trong giỏ hàng nếu lớn hơn 0
            <span className="bg-white text-black px-2 py-1 rounded-full">{cartCount}</span>
          )}
        </button>
        ...
        {user?.username && (
          ...
          >
            ...
            {newNotification && ( // Hiển thị biểu tượng thông báo nếu có thông báo mới
              <span className="bg-red-500 text-white px-2 py-1 rounded-full">!</span>
            )}
          </button>
        )}
